...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired,*short_processes, arrays[3];
...
static inline void activate_task(task_t *p, runqueue_t *rq)
{
	...
	if(p -> policy == SCHED_SHORT){
		...
		enqueue_task(p, array); //normal case
	}
	rq->nr_running++;
}
...
repeat_lock_task:
	...
	if (!p->array) {
		 ...
		 */
		 /* -------------------------------- HW2 ------------------------------------
		 try_to_wake_up()
		 * 1) A real-time process (p) returned from waiting and is ready to run.
         * process (p) has returned from waiting and has a higher priority.
         * 2) Another SHORT process (p) entered the run_queue with a highter priority
		 */
		task_t * current_proc = rq->curr;
		if(current_proc -> policy == SCHED_SHORT && p->policy != SCHED_OTHER && p->policy != SCHED_SHORT && current_proc -> state == TASK_RUNNING){ //case1
			resched_task(rq->curr);
		}
	    if(current_proc -> policy == SCHED_SHORT && p->policy == SCHED_SHORT && p -> short_prio < current_proc -> short_prio && current_proc -> state == TASK_RUNNING){ //case2
			resched_task(rq->curr);
		}  
		else if (p->prio < rq->curr->prio && current_proc -> policy != SCHED_SHORT) //that was the original if before changing
			resched_task(rq->curr);
		success = 1;
	}
	p->state = TASK_RUNNING;
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (p->array != rq->active && p -> array != rq ->short_processes ) { //if modified for short proccess
		set_tsk_need_resched(p);
		return;
	}
	spin_lock(&rq->lock);
	...
	if(p->policy==SCHED_SHORT)
	{
		if (p->sleep_avg) //added
			p->sleep_avg--; //added
		if(!--p->time_slice){
			p->isActivated=0;
			dequeue_task(p, rq->short_processes);
			p->policy=SCHED_OTHER;
			p->isPunished=1; //process is punished
			set_tsk_need_resched(p);
			p->prio = effective_prio(p); //with penalty
...
pick_next_task:
	...
	//--------------HW -----first if added second If modified and the else after was the original if before change
	if(rq->short_processes && rq->short_processes->nr_active>0){
		if(sched_find_first_bit((rq->active)->bitmap) >= 100){
			array=rq->short_processes;
		}
		else{
			array = rq->active;
...
/*
 * This has been replaced by sys_fsetpriority
 .  Maybe it should be
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	runqueue_t *rq;
	task_t *p;
	int tmp_time_slice; //for hw
	double double_time_slice;//for hw
	if (!param || pid < 0)
		goto out_nounlock;
	...
	if (policy < 0)
		...
		retval = -EINVAL;
		//----------HW2 If changed it means if policy isn't a legal value -------
	 ...
	 */
	retval = -EINVAL;
	//--------- HW2 code has been modified------		
	if((policy == SCHED_SHORT) && (lp.sched_short_prio <0 || lp.sched_short_prio >MAX_PRIO-1 )){
		goto out_unlock;
	}		
	if((policy == SCHED_SHORT) && (param -> requested_time <1 || param -> requested_time > 3000) ){
		goto out_unlock; // not sure of how to set errno to einval and return -1
	}
	...
	retval = 0;
	//-------HW2
	if(policy != SCHED_SHORT) //if added
		p->policy = policy;
	//------ENDHW2
	...
	if (policy != SCHED_OTHER && policy != SCHED_SHORT)
		...
		if (p->policy == SCHED_OTHER ){ // should always be true at this point 
			double_time_slice =(param -> requested_time)*(HZ/1000.0);
			tmp_time_slice = (int)double_time_slice;
			if(tmp_time_slice < MIN_TIMESLICE)
				tmp_time_slice = MIN_TIMESLICE;
			p -> time_slice = tmp_time_slice;
			p -> requested_time = param -> requested_time;
			p -> short_prio = param -> sched_short_prio;
	...
	if (array){
		...
		if(p->policy == SCHED_SHORT){
			p -> isActivated= 1;
			if(current ->policy ==SCHED_OTHER || (current->policy==SCHED_SHORT && p->short_prio < current->short_prio)
			|| (pid==current->pid && policy==SCHED_SHORT)) //not sure of that
...
asmlinkage long sys_sched_setparam(pid_t pid, struct sched_param *param)
{
	...
	if(p->policy == SCHED_SHORT)
		return -EPERM;
	else return setscheduler(pid, -1, param);
}
...
void __init sched_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		for (j = 0; j < 3; j++) { // for extended to 3 insted of 2
			array = rq->arrays + j;
...
int sys_is_short(pid_t pid){
	...
	//If no process with the corresponding PID exists
	if(p == NULL){
		return -ESRCH;
	}
	if( p->policy == SCHED_SHORT && p->isActivated==1){
		return 1; 
	}
	else 
		return 0;
...
int sys_short_remaining_time(pid_t pid){
	...
	//If the given proccess isn't a SHORT proccess
	if(p->policy!=SCHED_SHORT){
		return -EINVAL;
	}
	return (p->time_slice * 1000 /HZ); //the remaining time in ms before it gets converted back to OTHER
}


int sys_short_place_in_queue(pid_t pid){
	int shortProcessesNUM=0;
	int until=MAX_PRIO;
	int i;
	runqueue_t * rq;
	task_t* p = find_task_by_pid(pid);
	...
	//If the given proccess is the same as current proccess then the return value should be 0
	if(current->pid == pid){
		return 0;
	}
	rq = this_rq();
	for(i=0;i< until;i++){ //should make sure if until or until-1
		if(rq ->short_processes->queue+i){
			list_for_each(tmp ,rq ->short_processes->queue+i){
				
				if((list_entry(tmp,task_t,run_list)) -> pid!=pid){
					shortProcessesNUM++;
				}
				else
					goto out_short;	
